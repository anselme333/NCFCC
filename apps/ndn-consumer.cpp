...
Consumer::OnData(shared_ptr<const Data> data)
{
  ...
  ////////////////////////////////////////////////////
   ...
   Time VRTT=m_rtt-> m_currentEstimatedRtt;
   Time rto = m_rtt->RetransmitTimeout();
   Time RTTV=VRTT/2;
   Time FRTO= RTTV + max(RTT,VRTT);
   rto=FRTO;
   //////////////////////////////////////////////////////////////////////////////////////////
 }
 ...
 ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 // added by anselme
 void Consumer::OnRsr(std::shared_ptr<const ndn::Interest> ReduceSendRate)
 {
 	  App::OnRsr(ReduceSendRate); // tracing inside
 	  NS_LOG_FUNCTION(this << ReduceSendRate);
 	  Name prefix = ReduceSendRate->getName();
 	  if(prefix=="/RSR")
 	  {
 	  NS_LOG_FUNCTION(ReduceSendRate);
 	  ScheduleNextPacket();
 	  }
 }
...
void Consumer::OnTimeout1(uint32_t sequenceNumber)
{
  NS_LOG_FUNCTION(sequenceNumber);
  // std::cout << Simulator::Now () << ", TO: " << sequenceNumber << ", current RTO: " <<
  // m_rtt->RetransmitTimeout ().ToDouble (Time::S) << "s\n";
...
void
Consumer::fast_retransmission(uint32_t sequenceNumber)

{
	  using std::max;
	  Time now = Simulator::Now();
	  NS_LOG_FUNCTION(sequenceNumber);
	  Time RTT=m_rtt-> m_currentEstimatedRtt;
	  Time VRTT=m_rtt-> m_currentEstimatedRtt;
	  Time rto =3 * m_rtt->RetransmitTimeout();
	  m_rtt->SentSeq(SequenceNumber32(sequenceNumber),1);
	  m_retxSeqs.insert(sequenceNumber);
	  ScheduleNextPacket();
	  m_retxEvent = Simulator::Schedule(rto, &Consumer::CheckRetxTimeout, this);
}
void
